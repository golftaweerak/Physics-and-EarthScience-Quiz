    ...
    return `
        <a href="${linkUrl}" data-storage-key="${storageKey}" data-total-questions="${totalQuestions}" data-quiz-title="${quiz.title}" class="quiz-menu-item group block px-4 py-2 text-sm text-gray-700 dark:text-gray-200 hover:bg-gray-100 dark:hover:bg-gray-700 rounded-md transition-colors duration-200 border-l-2 border-transparent hover:border-blue-400 ${activeClass}">
            <div class="flex items-center gap-3">
                <div class="h-8 w-8 rounded-full flex items-center justify-center shrink-0 bg-white dark:bg-gray-200 p-1 transition-all duration-300 group-hover:scale-110 ${iconContainerExtraClass} overflow-hidden" style="min-width: 2rem; min-height: 2rem;">
...
export async function initializeMenu() {
    const runMenuInit = async () => {
        ...
        if (!menuDropdown || !menuQuizListContainer) {
            return false; // Indicating not ready
        }

        // Prevent double init if already populated (simple check)
        if (menuQuizListContainer.dataset.menuInitialized === 'true') return true;
        menuQuizListContainer.dataset.menuInitialized = 'true';
                    ...
                    <summary class="flex items-center justify-between p-2 rounded-md cursor-pointer hover:bg-gray-100 dark:hover:bg-gray-700 transition-colors">
                        <div class="flex items-center gap-2">
                            <div class="shrink-0 h-5 w-5" style="min-width: 1.25rem; min-height: 1.25rem;">
                                <img src="${categoryIconUrl}" alt="${details.title}" class="h-full w-full object-contain">
                            </div>
        ...
        // Ensure we don't attach duplicate listeners if we re-run
        if (!menuDropdown.dataset.hasListener) {
            ...
            menuDropdown.dataset.hasListener = 'true';
        }

        // --- Modal Button Listeners ---
        if (viewResultsBtn && !viewResultsBtn.dataset.hasListener) {
            ...
            viewResultsBtn.dataset.hasListener = 'true';
        }
        if (startOverBtn && !startOverBtn.dataset.hasListener) {
            startOverBtn.addEventListener('click', () => {
                if (activeStorageKey) localStorage.removeItem(activeStorageKey);
                if (activeQuizUrl) window.location.href = activeQuizUrl;
                completedModal.close();
            });
            startOverBtn.dataset.hasListener = 'true';
        }

        return true;
    };

    // Attempt to run init immediately.
    // App-loader calls this AFTER the header is loaded, so it should succeed.
    const success = await runMenuInit();
    if (!success) {
        console.warn("⚠️ MenuHandler: initializeMenu failed (elements not found). Ensure header is loaded first.");
    }
}
