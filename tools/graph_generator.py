...
def plot_phy_m4_ch6_6_q21():
    """
    Generates the graph for phy_m4_ch6-6, question 21.
    Description: A trapezoidal graph of Force vs. Time.
    Points: (0,0) -> (2,20) -> (8,20) -> (10,0).
    """
    fig, ax = plt.subplots()
    x = [0, 2, 8, 10]
    y = [0, 20, 20, 0]
    ...
    # Save the figure
    output_path = os.path.join('assets', 'images', 'phy_m4_ch6-6_q21.png')
    plt.savefig(output_path)
    plt.close(fig)
    print(f"Graph saved to {output_path}")
