...
export function convertTerm1Scores() {
  // --- Configuration ---
  ...
  const xlsxFilePath = path.join(__dirname, '..', 'xlsx', '69-EarthScience-Term1.xlsx');
  const outputJsFilePath = path.join(__dirname, '..', 'data', 'scores-data-1-2569.js');

  console.log('Starting Term 1 2569 score conversion script...');
  console.log(`Looking for input file at: ${xlsxFilePath}`);

  try {
    // 1. Read XLSX file
    if (!fs.existsSync(xlsxFilePath)) {
      throw new Error(`Input file not found. Please ensure '${path.basename(xlsxFilePath)}' is inside the 'xlsx' folder.`);
    }
    ...
    for (const row of rows) {
      ...
      // Handle both 'surname' and 'surmane' due to typo in some files
      const lastName = String(row[headerMap['surname']] || row[headerMap['surmane']] || '').trim();
      finalStudent.id = studentId; // Already validated
      finalStudent.name = `${title}${firstName} ${lastName}`.trim().replace(/\s+/g, ' '); // Combine and clean up whitespace
 ...
 *              Modify xlsx/${path.basename(xlsxFilePath)} and run 'node tools/fetch-term1-2569.js' instead.
 */
...
`;
    ...
    console.error(`\nPlease ensure that \`xlsx/${path.basename(xlsxFilePath)}\` exists and is formatted correctly.`);
  }
}